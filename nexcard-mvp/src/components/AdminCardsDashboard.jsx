...
import { CreditCard, Archive, ShieldBan, Link as LinkIcon, Loader2, CheckCircle2, AlertCircle, Search, Clock3, Filter } from 'lucide-react';
  ...
  const filteredRows = useMemo(() => {
    ...
    return rows.filter((card) => {
      const matchesStatus = statusFilter === 'all'
        ? true
        : card.status === statusFilter || card.activation_status === statusFilter;
          ...
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
            <label className="relative block">
              ...
              <input
                ...
                placeholder="Buscar por código, token, profile o evento"
            ...
            <label className="relative block">
              ...
              <select
                ...
                className="w-full appearance-none rounded-2xl border border-zinc-200 bg-white py-3 pl-10 pr-10 text-sm font-medium text-zinc-700 shadow-sm outline-none transition focus:border-zinc-400 sm:w-52"
              >
                {availableStatuses.map((status) => (
                  <option key={status} value={status}>
                    {status === 'all' ? 'Todos los status' : formatLabel(status)}
                  </option>
                ))}
              </select>
            </label>
                    ...
                    <tr key={card.id} className="hover:bg-zinc-50/30 transition-colors align-top">
                      ...
                      <td className="px-8 py-5">
                        <div className="flex flex-wrap gap-2">
                          {flags.map((flag) => (
                            <span key={flag} className="inline-flex rounded-full bg-zinc-100 px-2.5 py-1 text-[11px] font-black uppercase tracking-wide text-zinc-600">
                              {flag}
                            </span>
                          ))}
                        </div>
                ...
                {filteredRows.length === 0 && (
                  <tr>
                    <td colSpan={9} className="px-8 py-12 text-center text-sm font-semibold text-zinc-500">
                      No hay tarjetas que coincidan con los filtros activos.
                    </td>
                  </tr>
                )}
              </tbody>
